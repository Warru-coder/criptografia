...
class EncryptionService(
    ...
    fun deriveKeyFromPassword(password: CharArray, salt: ByteArray): ByteArray {
        ...
        for (i in 0 until ARGON2_ITERATIONS) {
            derived = pbkdf2HMAC(derived, salt, 10000, AES_KEY_SIZE)
        }
    ...
    private fun pbkdf2HMAC(password: ByteArray, salt: ByteArray, iterations: Int, keyLength: Int): ByteArray {
        val mac = javax.crypto.Mac.getInstance("HmacSHA256")
        mac.init(javax.crypto.spec.SecretKeySpec(password, "HmacSHA256"))
        
        val blocks = (keyLength + 31) / 32
        val derivedKey = ByteArray(keyLength)
        
        for (i in 1..blocks) {
            val u = mac.doFinal(salt + byteArrayOf((i shr 24).toByte(), (i shr 16).toByte(), (i shr 8).toByte(), i.toByte()))
            var t = u.copyOf()
            
            for (j in 1 until iterations) {
                val uNext = mac.doFinal(u)
                for (k in u.indices) {
                    t[k] = t[k] xor uNext[k]
                }
                u = uNext
            ...
            System.arraycopy(t, 0, derivedKey, (i - 1) * 32, minOf(t.size, keyLength - (i - 1) * 32))
        }
