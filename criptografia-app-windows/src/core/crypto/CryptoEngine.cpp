...
CryptoEngine::CryptoEngine() : m_hAesAlg(NULL), m_hHashAlg(NULL), m_hPbkdf2Alg(NULL) {
    ...
    // HMAC-SHA256 provider used as PRF for BCryptDeriveKeyPBKDF2. No weak
    // fallback: if this fails, the engine must not start.
...
std::vector<BYTE> CryptoEngine::DeriveKey(const std::wstring& password, const std::vector<BYTE>& salt, size_t keyLength) {
    ...
    // UTF-8 encode the password so the KDF input matches the other platforms
    ...
    NTSTATUS status = BCryptDeriveKeyPBKDF2(
        m_hPbkdf2Alg,
        passwordBytes.empty() ? NULL : passwordBytes.data(), static_cast<ULONG>(passwordBytes.size()),
        const_cast<BYTE*>(salt.data()), static_cast<ULONG>(salt.size()),
